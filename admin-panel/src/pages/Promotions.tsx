...
export function CreateAdModal({ open, onClose, governorates, settings, onCreated }: { open: any; onClose: any; governorates: any; settings: any; onCreated: any }) {
  ...
  const [form, setForm] = useState({ provider_id: '', item_id: '', duration_days: settings?.duration_days || 7, target: 'governorate', governorate_ids: [] as number[], billing: 'wallet' });
  const [items, setItems] = useState<any>(null);
  const [saving, setSaving] = useState(false);

  useEffect(() => {
    if (!open) return;
    setForm({ provider_id: '', item_id: '', duration_days: settings?.duration_days || 7, target: 'governorate', governorate_ids: [] as number[], billing: 'wallet' });
    setItems(null);
    setBalances({});
...
export default function Promotions() {
  ...
  useEffect(() => { load(); }, [page, status, governorateId, serviceId, qApplied]);
  useEffect(() => { setPage(1); }, [status, governorateId, serviceId, qApplied]);
