...
app.use('/uploads', (req, res, next) => {
  ...
  const sensitive = get(
    `SELECT 1 AS one FROM providers WHERE national_id_image = ? OR residency_doc_image = ?
     UNION ALL SELECT 1 FROM recharge_requests WHERE proof_image = ? LIMIT 1`,
    [ref, ref, ref]
  );
  if (sensitive) return res.status(404).json({ message: 'الملف غير موجود' });
