...
function logActivity(user, action, entityType, entityId, details, req = null) {
  try {
    run(
      'INSERT INTO activity_log (user_id, actor_role, action, entity_type, entity_id, details, ip_address, user_agent) VALUES (?,?,?,?,?,?,?,?)',
      [
        user ? user.id : null,
...
function pruneActivityLog() {
  try {
    ...
    if (res?.changes > 0) console.log(`[log] تقليم activity_log: حُذف ${res.changes} سجلاً أقدم من ${days} يوم`);
  } catch (e: any) {
