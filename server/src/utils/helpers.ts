...
function parseDateRange(from, to) {
  ...
  if (fromUtc && toUtc && fromUtc > toUtc) throw new ApiError(400, 'تاريخ البداية يجب ألا يتجاوز تاريخ النهاية');
  return { fromUtc, toUtc };
}

// هل عقد إيجار الوكيل نشط؟ (موحّد في مكان واحد لمنع التباين بين الخدمات)
function isAgentLeaseActive(agent) {
  if (!agent) return false;
  return agent.lease_status === 'active'
    && !!agent.lease_expires_at
    && new Date(agent.lease_expires_at) > new Date();
}

// يتحقق من صحة نسبة (0..100)؛ القيم الفارغة/الملغاة تُعتبر صالحة (تُترك لاستخدام الافتراضي)
...
module.exports = { ApiError, toId, round2, nextLeasePeriod, settingValue, paginate, csvEscape, assertLength, isUniqueViolation, localDayUtcBoundary, parseDateRange, parseImages, parseIncludes, isAgentLeaseActive, MAX_MONEY, MAX_QUANTITY, assertAmount, assertQuantity, validRate };
