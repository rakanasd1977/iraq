...
function readCentral(zip) {
  ...
  for (let i = zip.length - 22; i >= tailStart; i--) {
    if (zip.readUInt32LE(i) === SIG_EOCD) { eocdPos = i; break; }
  }
  if (eocdPos < 0) throw new Error('أرشيف غير صالح: EOCD غير موجود');
  ...
  while (pos < end && out.length < entryCount) {
    if (zip.readUInt32LE(pos) !== SIG_CENTRAL) throw new Error('أرشيف غير صالح: سجل مركزي خاطئ');
...
function extractEntry(zip, entry) {
  ...
  if (zip.readUInt32LE(local) !== SIG_LOCAL) throw new Error('أرشيف غير صالح: سجل محلي خاطئ');
  ...
  if (data.length !== entry.usize) throw new Error('أرشيف غير صالح: حجم البيانات لا يطابق');
  if (crc32(data) !== entry.crc) throw new Error('أرشيف غير صالح: CRC لا يطابق (ملف تالف)');
  return data;
}
...
function extractZip(zip, destDir) {
  ...
  for (const entry of readCentral(zip)) {
    ...
    if (!abs.startsWith(root + path.sep)) throw new Error('إدخال غير آمن في الأرشيف: ' + entry.name);
