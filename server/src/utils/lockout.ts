...
function recentFailures(identifier, ip) {
  ...
  const byIdentifier = get(
    'SELECT COUNT(*) AS c FROM login_failures WHERE identifier = ? AND created_at > ?',
    [identifier, since]
  );
  const byIp = get(
...
function lockRemaining(identifier, ip) {
  ...
  if (!hitMax) return null;
  // أول محاولة تجاوزت العتبة حددت بداية القفل؛ نحسب الباقي من أحدث قيد داخل النافذة
  const latest = get(
    'SELECT MAX(created_at) AS m FROM login_failures WHERE (identifier = ? OR ip = ?) AND created_at > ?',
    [identifier, ip, iso(Date.now() - WINDOW_MS)]
  );
