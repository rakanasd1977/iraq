...
function recomputeItemRating(itemType, itemId, providerId) {
  ...
  run(
    `INSERT INTO item_rating_sums (item_type, item_id, rating, rating_count) VALUES (?,?,?,?)
     ON CONFLICT(item_type, item_id) DO UPDATE SET rating = excluded.rating, rating_count = excluded.rating_count`,
    [itemType, itemId, rating, count]
  );
...
function hasPurchasedItem(userId, kind, itemId) {
  const orders = all(
    'SELECT id, items_json FROM orders WHERE customer_id = ? AND status = ? ORDER BY id DESC LIMIT 50',
    [userId, 'completed']
  );
  for (const o of orders) {
    ...
    if (items.some((it) => it && it.kind === kind && Number(it.item_id) === Number(itemId))) return o.id;
  }
  return null;
}
