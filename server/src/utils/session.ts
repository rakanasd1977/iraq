...
function issueToken(user, req) {
  ...
  run(
    'INSERT INTO sessions (id, user_id, expires_at, user_agent, ip, fingerprint) VALUES (?,?,?,?,?,?)',
    [jti, user.id, expiresAt, String(req.headers['user-agent'] || '').slice(0, 200), clientIp(req), deviceFingerprint(req)]
  );
...
function pruneSessions() {
  try {
    ...
    if (res?.changes > 0) console.log(`[session] تقليم الجلسات المنتهية: حُذف ${res.changes} جلسة`);
  } catch (e: any) {
