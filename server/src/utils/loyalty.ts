...
function planRedeem(userId, requested, maxDiscount) {
  ...
  if (balance < minRedeem) throw new ApiError(400, 'رصيد النقاط أقل من الحد الأدنى للاستبدال');
...
function grantReferralRewards(order) {
  ...
  if (Number(order.total_amount) < settingValue('referral_min_order', 10000)) return;
