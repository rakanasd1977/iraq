...
const { round2, ApiError } = require('../utils/helpers');
...
function getAdminDashboard() {
  ...
  const revenue = get(
    `SELECT COUNT(*) AS orders_count, COALESCE(SUM(o.total_amount),0) AS orders_value,
  ...
  const byService = all(
    `SELECT s.id, s.slug, s.name_ar, s.icon, COUNT(DISTINCT p.id) AS providers_count,
  ...
  const byGovernorate = all(
    `SELECT g.id, g.name_ar, g.code, COUNT(DISTINCT p.id) AS providers_count, COUNT(o.id) AS orders_count,
...
function getAgentDashboard(actor) {
  const agent = get('SELECT * FROM agents WHERE id = ?', [actor.agent_id]);
  if (!agent) return {};
  const gov = get('SELECT * FROM governorates WHERE id = ?', [agent.governorate_id]);

  const orders = get(
    `SELECT COUNT(*) AS c, COALESCE(SUM(o.total_amount),0) AS value,
            COALESCE(SUM(o.agent_amount),0) AS agent_revenue,
            COALESCE(SUM(o.platform_amount),0) AS platform_revenue
  ...
  const byStatus = all(
    `SELECT o.status, COUNT(*) AS count FROM orders o
     JOIN providers p ON p.id = o.provider_id WHERE p.governorate_id = ? GROUP BY o.status`,
    [gov.id]
  );
...
function getExecutiveDashboard(query) {
  const period = String((query && query.period) || 'month').trim();
  const validPeriods = ['day', 'week', 'month'];
  if (!validPeriods.includes(period)) throw new ApiError(400, 'فترة غير صالحة');
  ...
  function kpiQuery(table, dateCol, whereExtra = '', paramsExtra = [], valueCol = 'total_amount') {
    const valExpr = valueCol ? `COALESCE(SUM(${valueCol}),0)` : '0';
    ...
    const curRow = get(`SELECT COUNT(*) AS c, ${valExpr} AS v FROM ${table} WHERE ${where}`, curParams);
    const prevRow = get(`SELECT COUNT(*) AS c, ${valExpr} AS v FROM ${table} WHERE ${where}`, prevParams);
    return { cur: curRow, prev: prevRow };
  }

  const ordersKpi = kpiQuery('orders', 'created_at', "status != 'cancelled'");
  const customersKpi = kpiQuery('users', 'created_at', "role = 'customer'", [], null);
  const revenueKpi = kpiQuery('orders', 'created_at', "status != 'cancelled'");
  const completedCur = get(`SELECT COUNT(*) AS c FROM orders WHERE created_at >= ? AND created_at < ? AND status = 'completed'`, [cur.start, cur.end]).c;
  const completedPrev = get(`SELECT COUNT(*) AS c FROM orders WHERE created_at >= ? AND created_at < ? AND status = 'completed'`, [prev.start, prev.end]).c;
  ...
  const kpis = {
    ...
    conversion: {
      label: 'نسبة الإنجاز (%)',
      current: ordersKpi.cur.c > 0 ? round2((completedCur / ordersKpi.cur.c) * 100) : 0,
      previous: ordersKpi.prev.c > 0 ? round2((completedPrev / ordersKpi.prev.c) * 100) : 0,
      deltaPct: round2((ordersKpi.cur.c > 0 ? (completedCur / ordersKpi.cur.c) * 100 : 0) - (ordersKpi.prev.c > 0 ? (completedPrev / ordersKpi.prev.c) * 100 : 0)),
    },
  };
  ...
  const aovCur = ordersKpi.cur.c > 0 ? round2(revenueKpi.cur.v / ordersKpi.cur.c) : 0;
  const aovPrev = ordersKpi.prev.c > 0 ? round2(revenueKpi.prev.v / ordersKpi.prev.c) : 0;

  const statusBreakdown = ['pending', 'confirmed', 'completed', 'cancelled'].map((st) => {
    const row = get(`SELECT COUNT(*) AS c, COALESCE(SUM(total_amount),0) AS v FROM orders WHERE created_at >= ? AND created_at < ? AND status = ?`, [cur.start, cur.end, st]);
    return { status: st, count: row.c, revenue: round2(row.v) };
  });

  const topProvinces = all(
    `SELECT g.id AS id, g.name_ar AS name, COUNT(o.id) AS orders, COALESCE(SUM(o.total_amount),0) AS revenue
     FROM orders o JOIN governorates g ON g.id = o.governorate_id
     WHERE o.created_at >= ? AND o.created_at < ?
     GROUP BY g.id ORDER BY orders DESC LIMIT 5`,
    [cur.start, cur.end]
  ).map((r) => ({ id: r.id, name: r.name, orders: r.orders, revenue: round2(r.revenue) }));

  const topProviders = all(
    `SELECT p.id AS id, p.name_ar AS name, COUNT(o.id) AS orders, COALESCE(SUM(o.total_amount),0) AS revenue
     FROM orders o JOIN providers p ON p.id = o.provider_id
     WHERE o.created_at >= ? AND o.created_at < ?
     GROUP BY p.id ORDER BY orders DESC LIMIT 5`,
    [cur.start, cur.end]
  ).map((r) => ({ id: r.id, name: r.name, orders: r.orders, revenue: round2(r.revenue) }));

  const attention = {
    pendingProviders: get(`SELECT COUNT(*) AS c FROM providers WHERE is_verified = 0`).c,
    pendingAgentWithdrawals: get(`SELECT COUNT(*) AS c FROM agent_withdrawals WHERE status = 'pending'`).c,
  };
  ...
  return {
    ...
    averages: { aov: aovCur, aovPrevious: aovPrev },
    statusBreakdown,
    topProvinces,
    topProviders,
    attention,
...
module.exports = { monthlySeries, getAdminDashboard, getAgentDashboard, getExecutiveDashboard };
