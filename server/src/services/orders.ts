...
const { ApiError, toId, round2, paginate, csvEscape, isUniqueViolation, localDayUtcBoundary, assertAmount, assertQuantity, settingValue, isAgentLeaseActive } = require('../utils/helpers');
    ...
    if (status === 'confirmed') {
      ...
      if (isFreeOrder) {
        ...
        run(
          ...
          [order.provider_id, 'commission', 0, 0, 0, Number(wb ? wb.balance : 0), order.id, order.order_number,
            `طلب مجاني ضمن العرض الترحيبي (${acceptedCount + 1}/${freeLimit}) — لا عمولة للمنصة ولا للوكيل`,
