...
function insertPromotionRow({ provider, def, item, durationDays, cost, targetType, governorateIds, billing, endsAt }) {
  ...
  return run(
    ...
    [provider.id, provider.service_id, def.table, item.id, item[def.titleCol], round2(item[def.priceCol]),
     firstImage(item[def.imageCol]), def.kind, representativeGovId, targetType, idsCsv, billing, cost, endsAt]
  ).lastId;
}
...
function buildPromotionFilters(query) {
  const { status, governorate_id, service_id, q } = query || {};
  const where = [];
  const params = [];
...
function createAdminPromotion(user, body) {
  const { provider_id, item_type, item_id, duration_days, billing } = body || {};
  const def = ITEM_TYPES[item_type];
  if (!def) throw new ApiError(400, 'نوع العنصر غير صحيح');
...
function listPublicPromotions(query) {
  const { governorate_code, governorate_id } = query || {};
