...
function listCustomers(query) {
  ...
  const params = [];
  const baseJoin = 'FROM customers c JOIN users u ON u.id = c.user_id LEFT JOIN governorates g ON g.id = c.governorate_id';
  const qWhere = q ? ' WHERE (u.name_ar LIKE ? OR u.email LIKE ? OR u.phone LIKE ?)' : '';
  if (q) params.push(`%${q}%`, `%${q}%`, `%${q}%`);

  const pg = paginate({ query }, 50);
  if (pg.enabled) {
    const total = get(`SELECT COUNT(*) AS c ${baseJoin} ${qWhere}`, q ? [`%${q}%`, `%${q}%`, `%${q}%`] : []).c;
    const rows = all(sql + (q ? ` WHERE (u.name_ar LIKE ? OR u.email LIKE ? OR u.phone LIKE ?)` : '') + ' ORDER BY c.id DESC LIMIT ? OFFSET ?', [...params, pg.limit, pg.offset]);
    return { rows, meta: { total, page: pg.page, limit: pg.limit, pages: Math.max(1, Math.ceil(total / pg.limit)) } };
  }
  return { rows: all(sql + ' ORDER BY c.id DESC', params) };
}

function getCustomer(id) {
  id = toId(id);
  const customer = get(CUSTOMER_SELECT + ' WHERE c.id = ?', [id]);
  if (!customer) throw new ApiError(404, 'الزبون غير موجود');
  return customer;
}

async function createCustomer(body, actor) {
  const { name_ar, name_en, email, phone, password, governorate_id, address } = body || {};
  ...
  transaction(() => {
    userId = run(
      ...
      ['customer', name_ar, name_en || null, String(email).toLowerCase(), phone, passwordHash, govId]
    ).lastId;
    customerId = run('INSERT INTO customers (user_id, governorate_id, address) VALUES (?,?,?)', [userId, govId, address || null]).lastId;
  });

  logActivity(actor, 'create', 'customer', customerId, { name_ar });
  return get(CUSTOMER_SELECT + ' WHERE c.id = ?', [customerId]);
}

function updateCustomer(id, body, actor) {
  id = toId(id);
  const customer = get('SELECT * FROM customers WHERE id = ?', [id]);
  if (!customer) throw new ApiError(404, 'الزبون غير موجود');
  const user = get('SELECT * FROM users WHERE id = ?', [customer.user_id]);

  const { name_ar, name_en, email, phone, governorate_id, address, is_active } = body || {};
  if (email) {
  ...
  logActivity(actor, 'update', 'customer', id, { name_ar });
  return get(CUSTOMER_SELECT + ' WHERE c.id = ?', [id]);
}

function deleteCustomer(id, actor) {
  id = toId(id);
  const customer = get('SELECT * FROM customers WHERE id = ?', [id]);
  if (!customer) throw new ApiError(404, 'الزبون غير موجود');
  ...
  if (orders > 0) {
    ...
    return { message: 'لا يمكن حذف الزبون لوجود طلبات، تم إيقافه بدلاً من ذلك' };
  }
  transaction(() => {
    run('DELETE FROM customers WHERE user_id = ?', [customer.user_id]);
    run('DELETE FROM users WHERE id = ?', [customer.user_id]);
  });
  logActivity(actor, 'delete', 'customer', id, { name_ar: customer.name_ar });
  return { message: 'تم حذف الزبون بنجاح' };
}
