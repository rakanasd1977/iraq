...
function listDistricts(query) {
  ...
  const agentsByDistrict = {};
  all(`SELECT a.district_id, a.id, a.lease_status, a.lease_expires_at, a.commission_rate,
              u.name_ar AS agent_name, u.email AS agent_email, u.is_active AS agent_active
       FROM agents a JOIN users u ON u.id = a.user_id`).forEach((a) => {
    if (!agentsByDistrict[a.district_id]) agentsByDistrict[a.district_id] = a;
  });
  return rows.map((d) => ({ ...d, agent: agentsByDistrict[d.id] || null }));
}
