...
const { ApiError, nextLeasePeriod, round2, paginate, parseDateRange } = require('../utils/helpers');
...
function resolveActiveAgent(user) {
  ...
  if (!active) throw new ApiError(403, 'إجارة الوكالة منتهية أو قيد الموافقة — لا يمكن تنفيذ هذا الإجراء');
  return agent;
}
