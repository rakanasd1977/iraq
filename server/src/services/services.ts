...
const { ApiError, toId, validRate } = require('../utils/helpers');
...
function listServices() {
  const rows = all(`SELECT s.*, COALESCE(pc.c, 0) AS providers_count
    FROM services s
    LEFT JOIN (SELECT service_id, COUNT(*) AS c FROM providers GROUP BY service_id) pc ON pc.service_id = s.id
