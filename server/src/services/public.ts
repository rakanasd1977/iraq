...
function listProviders(query, locale = 'ar') {
  ...
  if (rows.length) {
    ...
    const countByTable = (table) =>
      all(`SELECT provider_id, COUNT(*) AS c FROM ${table} WHERE provider_id IN (${ids}) AND is_active = 1 GROUP BY provider_id`);
    for (const table of ['products', 'menu_items', 'hotel_rooms', 'flights', 'travel_packages']) {
      for (const row of countByTable(table)) countsMap[`${row.provider_id}:${table}`] = row.c;
    }
  }
...
function getProviderProducts(id, query, locale = 'ar') {
  ...
  if (sort === 'sold') rows = rows.sort((a, b) => b.sold - a.sold || b.id - a.id);
...
module.exports = {
  ...
  getItemReviews, getDeals, getGovernorateByGeo,
};
