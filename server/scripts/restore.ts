...
function pickBackup() {
  ...
  if (explicit) {
    if (!fs.existsSync(explicit)) fail(`الملف غير موجود: ${explicit}`);
    return path.resolve(explicit);
  }
  if (!fs.existsSync(BACKUP_DIR)) fail('لا يوجد مجلد نسخ احتياطية: ' + BACKUP_DIR);
  const files = fs.readdirSync(BACKUP_DIR).filter((f) => STAMP_RE.test(f)).sort();
  if (!files.length) fail('لا توجد نسخ احتياطية في ' + BACKUP_DIR);
...
async function main() {
  ...
  try {
    if (isZip) {
      ...
      if (!entries.some((e) => e.name === 'app.db')) fail('الحزمة لا تضم قاعدة بيانات (app.db)');
      log('استخراج الحزمة...');
    ...
    if (!integrity(restoredDb)) fail('نسخة تالفة: integrity_check فشل — لم تُطبَّق الاستعادة');
    ...
    if (!integrity(DB_FILE)) fail('فشل التحقق بعد الاستعادة — راجع نسخة الأمان.');
