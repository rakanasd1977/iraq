...
async function main() {
    ...
    async function writeOrder() {
        const ms = await hit(() => fetch(`${BASE}/api/orders`, jsonBody('POST', `${BASE}/api/orders`, agentToken, {
        ...
        if (ms !== null)
            record('orders.create', ms, stats.orders?.last || 201);
    }
    async function cancelOrder() {
        const ms = await hit(() => fetch(`${BASE}/api/orders?page=1&limit=50`, { headers: { Authorization: `Bearer ${providerToken}` } })
            .then(async (list) => {
        ...
        if (ms !== null)
            record('orders.cancel', ms, stats.orders?.last || 200);
    }
    async function confirmOrder() {
        ...
        if (ms !== null)
            record('orders.confirm', ms, stats.orders?.last || 200);
    }
    async function rechargeWallet() {
        const ms = await hit(() => fetch(`${BASE}/api/wallets/${providerId}/recharge`, jsonBody('POST', `${BASE}/api/wallets/${providerId}/recharge`, adminToken, {
            amount: 1000 + rnd(100) * 100,
            note: 'شحن اختبار تحميل',
        })));
        if (ms !== null)
            record('wallets.recharge', ms, stats.orders?.last || 200);
    }
    async function markNotif() {
        const ms = await hit(() => fetch(`${BASE}/api/notifications?limit=5`, { headers: { Authorization: `Bearer ${agentToken}` } })
            .then(async (list) => {
        ...
        if (ms !== null)
            record('notifications.read', ms, stats.orders?.last || 200);
    }
    const reads = [
