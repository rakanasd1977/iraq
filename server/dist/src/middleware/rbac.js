...
function loadUserPermissions(userId) {
    const rows = all(`SELECT DISTINCT arp.resource, arp.action
...
function requireRole(...roles) {
    return (req, res, next) => {
        if (!req.user)
            return next(new ApiError(401, 'غير مصرح بالدخول'));
        if (!roles.includes(req.user.role))
            return next(new ApiError(403, 'لا تملك صلاحية تنفيذ هذه العملية'));
        next();
    };
}
function requireAdminOrAgent() {
    return (req, res, next) => {
        ...
        if (!['admin', 'agent'].includes(req.user.role))
            return next(new ApiError(403, 'لا تملك صلاحية تنفيذ هذه العملية'));
        if (req.user.role === 'agent' && !req.user.agent_id)
            return next(new ApiError(403, 'حساب الوكيل غير مكتمل'));
        next();
    };
}
...
function requireAgentLease() {
    return (req, res, next) => {
        ...
        if (!active)
            return next(new ApiError(403, 'إجارة الوكالة منتهية أو غير مفعلة، يرجى تجديدها قبل تنفيذ هذه العملية'));
        next();
    };
}
...
function requirePermission(resource, action) {
    return (req, res, next) => {
        ...
        if (req.user.role === 'admin') {
            ...
            const superAdmin = get(`SELECT 1 FROM admin_user_roles aur
         JOIN admin_roles ar ON ar.id = aur.role_id
         WHERE aur.user_id = ? AND ar.name = 'super_admin'`, [req.user.id]);
            if (superAdmin)
                return next();
        }
        // تحميل صلاحيات المستخدم (مع تخزين مؤقت بسيط في الطلب)
        if (!req._rbacPerms)
            req._rbacPerms = loadUserPermissions(req.user.id);
...
function loadPermissions(req, res, next) {
    if (req.user)
        req.permissions = loadUserPermissions(req.user.id);
    next();
}
module.exports = { requireRole, requireAdminOrAgent, requireAgentLease, requirePermission, loadPermissions };
