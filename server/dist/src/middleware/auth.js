...
const { readCookie, markCookieAuth } = require('../utils/csrf');
...
function authenticate(req, res, next) {
    ...
    if (!token)
        return next(new ApiError(401, 'غير مصرح بالدخول، يرجى تسجيل الدخول'));
    let payload;
    try {
    ...
    if (!user)
        return next(new ApiError(401, 'الحساب غير موجود'));
    if (!user.is_active)
        return next(new ApiError(403, 'هذا الحساب موقوف، يرجى التواصل مع الإدارة'));
