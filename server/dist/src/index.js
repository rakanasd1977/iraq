...
const WORKER_COUNT = Math.min(Number(process.env.CLUSTER_WORKERS) || os.cpus().length, Number(process.env.CLUSTER_WORKERS_MAX) || 4);
if (cluster.isPrimary) {
    const config = require('./config');
    const { migrate } = require('./db/migrate');
    const { checkpointAndBackup, pruneIdempotencyKeys, endExpiredPromotions, rotateLogs, maintenanceSnapshot, checkAdminNotifications } = require('./utils/maintenance');
    ...
    process.on('message', (msg) => {
        if (!msg || msg.type !== 'sse-deliver')
            return;
        if (msg.userId)
            publishLocal(msg.userId, msg.event, msg.data);
        else
            publishAllLocal(msg.event, msg.data);
    });
