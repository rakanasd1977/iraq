...
function attachWorker(w, port, key) {
    w.unref();
    w.on('error', (err) => {
        const pending = [...inflight.values()].filter((p) => p.worker === key);
...
function ensureWorkers() {
    ...
    function spawnWorker(mode) {
        ...
        // في الإنتاج (مُجمّع) يُحمَّل worker.js، وفي التطوير (tsx) يُحمَّل worker.ts
        ...
        const key = mode + '-' + readerWorkers.length;
        attachWorker(worker, channel.port1, key);
        const deadline = Date.now() + 15000;
        for (;;) {
            const msg = receiveMessageOnPort(channel.port1);
            if (msg && msg.message && msg.message.id === 'ready')
                break;
            if (Date.now() > deadline)
                throw new Error('db worker did not become ready: ' + mode);
            Atomics.wait(signal, 0, Atomics.load(signal, 0), 10);
        }
        return { worker, port: channel.port1, key };
    }
...
function trackTxDelta(method, sql) {
    ...
    if (method === 'exec' && sql) {
        ...
        if (s.startsWith('BEGIN'))
            return 1;
        if (s.startsWith('COMMIT') || s.startsWith('ROLLBACK') || s.startsWith('END'))
            return -1;
    }
    return 0;
}
function call(method, sql, params) {
    ...
    const key = port === writerPort ? 'writer' : 'reader';
    inflight.set(id, {
        id,
        worker: key,
        setResult(v) { value = v; },
    ...
    for (;;) {
        ...
        if (msg) {
            ...
            if (p) {
                ...
                if (m.ok)
                    p.setResult(m.result);
                else
                    p.setError(new Error(m.err));
            }
            if (m.id === id)
                break;
            continue;
...
function transaction(fn) {
    ...
    try {
        ...
        try {
            call('rollback', null, null);
        }
        catch (e2) { /* تجاهل فشل التراجع */ }
        throw e;
    }
}
const db = {
...
module.exports = { db, get, all, run, transaction, close };
