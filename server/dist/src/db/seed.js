...
migrate();
const GOVERNORATES = [
    { ar: 'بغداد', en: 'Baghdad', code: 'BAG', fee: 5000000 },
    { ar: 'البصرة', en: 'Basra', code: 'BAS', fee: 3000000 },
    { ar: 'نينوى', en: 'Nineveh', code: 'NIN', fee: 2500000 },
    { ar: 'أربيل', en: 'Erbil', code: 'ERB', fee: 3500000 },
    { ar: 'السليمانية', en: 'Sulaymaniyah', code: 'SUL', fee: 2500000 },
    { ar: 'دهوك', en: 'Duhok', code: 'DUH', fee: 2000000 },
    { ar: 'كركوك', en: 'Kirkuk', code: 'KIR', fee: 1500000 },
    { ar: 'النجف', en: 'Najaf', code: 'NAJ', fee: 2500000 },
    { ar: 'كربلاء', en: 'Karbala', code: 'KAR', fee: 2000000 },
    { ar: 'الأنبار', en: 'Anbar', code: 'ANB', fee: 1500000 },
    { ar: 'صلاح الدين', en: 'Salah al-Din', code: 'SAL', fee: 1200000 },
    { ar: 'ديالى', en: 'Diyala', code: 'DIY', fee: 1200000 },
    { ar: 'واسط', en: 'Wasit', code: 'WAS', fee: 1200000 },
    { ar: 'ذي قار', en: 'Dhi Qar', code: 'DHI', fee: 1200000 },
    { ar: 'ميسان', en: 'Maysan', code: 'MAY', fee: 1000000 },
    { ar: 'المثنى', en: 'Muthanna', code: 'MUT', fee: 800000 },
    { ar: 'القادسية', en: 'Al-Qadisiyyah', code: 'QAD', fee: 1000000 },
    { ar: 'بابل', en: 'Babylon', code: 'BAB', fee: 1500000 },
];
const SERVICES = [
...
const SETTINGS = {
    ...
    zain_cash_number: { value: '', label: 'رقم زين كاش لاستقبال الشحنات' },
    asia_pay_number: { value: '', label: 'رقم آسيا باي لاستقبال الشحنات' },
...
function seedGovernorates() {
    ...
    if (count > 0)
        return console.log('[seed] المحافظات موجودة مسبقاً');
    GOVERNORATES.forEach((g, i) => {
        run('INSERT INTO governorates (name_ar, name_en, code, lease_fee, sort_order) VALUES (?,?,?,?,?)', [g.ar, g.en, g.code, g.fee, i + 1]);
    });
...
function seedServices() {
    ...
    if (count > 0)
        return console.log('[seed] الخدمات موجودة مسبقاً');
    SERVICES.forEach((s, i) => {
        run('INSERT INTO services (slug, name_ar, name_en, icon, description, sort_order) VALUES (?,?,?,?,?,?)', [s.slug, s.ar, s.en, s.icon, s.desc, i + 1]);
    });
...
function seedAccounts() {
    ...
    if (admin)
        return console.log('[seed] حسابات المستخدمين موجودة مسبقاً');
    ...
    run('INSERT INTO agents (user_id, governorate_id, commission_rate, lease_status, lease_expires_at) VALUES (?,?,?,?,?)', [agentUserId, baghdad.id, 2, 'active', end.toISOString()]);
    ...
    run('INSERT INTO providers (user_id, governorate_id, service_id, name_ar, commission_rate, is_active, is_verified) VALUES (?,?,?,?,?,1,1)', [providerUserId, baghdad.id, stores.id, 'متجر الرافدين للتجارة', 5]);
    ...
    for (const a of otherProviders) {
        ...
        run('INSERT INTO providers (user_id, governorate_id, service_id, name_ar, commission_rate, is_active, is_verified) VALUES (?,?,?,?,?,1,1)', [uid, baghdad.id, svc.id, a.name, 5]);
...
function seedCoupons() {
    ...
    const store = get(`SELECT p.id FROM providers p
     JOIN services s ON s.id = p.service_id
     JOIN governorates g ON g.id = p.governorate_id
     WHERE s.slug = 'stores' AND g.code = 'BAG' ORDER BY p.id ASC LIMIT 1`);
    if (!store)
        return console.log('[seed] لا يوجد متجر تجريبي لربط الكوبون به');
    const existing = get('SELECT id FROM coupons WHERE code = ?', ['RAFIDAIN10']);
    if (existing)
        return console.log('[seed] كوبون RAFIDAIN10 موجود مسبقاً');
    run('INSERT INTO coupons (code, title, discount_type, discount_value, min_amount, provider_id, max_uses, per_customer_limit, is_active) VALUES (?,?,?,?,?,?,?,?,1)', ['RAFIDAIN10', 'خصم ترحيبي 10%', 'percent', 10, 0, store.id, 0, 1]);
...
function seedCatalog() {
    ...
    if (rest) {
        ...
        const addMenuItem = (categoryName, f) => {
            ...
            if (!cat)
                cat = { id: run('INSERT INTO menu_categories (provider_id, name_ar, sort_order) VALUES (?,?,?)', [rest.id, categoryName, 99]).lastId };
            const exists = get('SELECT id FROM menu_items WHERE provider_id = ? AND name_ar = ?', [rest.id, f.name_ar]);
            if (exists)
                return;
            run('INSERT INTO menu_items (provider_id, category_id, name_ar, name_en, description, price, images_json, is_active, is_featured, is_available) VALUES (?,?,?,?,?,?,?,1,?,1)', [rest.id, cat.id, f.name_ar, f.name_en, f.description, f.price, JSON.stringify([f.img]), f.is_featured]);
        };
    ...
    if (air) {
        ...
        const addFlight = (f) => {
            ...
            if (exists)
                return;
            run('INSERT INTO flights (provider_id, flight_number, airline, origin, origin_ar, destination, destination_ar, departure_at, arrival_at, price, seats, images_json, is_active, is_featured) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,1,?)', [air.id, f.flight_number, f.airline, f.origin, f.origin_ar, f.destination, f.destination_ar, f.departure_at, f.arrival_at, f.price, f.seats, JSON.stringify([IMG('photo-1436491865332-7a61a109cc05')]), f.is_featured]);
        };
    ...
    if (travel) {
        ...
        const addPackage = (f) => {
            ...
            if (exists)
                return;
            run('INSERT INTO travel_packages (provider_id, name_ar, name_en, description, destination, duration_days, price, includes_json, images_json, is_active, is_featured) VALUES (?,?,?,?,?,?,?,?,?,1,?)', [travel.id, f.name_ar, f.name_en, f.description, f.destination, f.duration_days, f.price, JSON.stringify(f.includes), JSON.stringify([f.img]), f.is_featured]);
        };
