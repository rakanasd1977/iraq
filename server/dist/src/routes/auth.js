...
const PRIVILEGED_ROLES = ['admin', 'agent', 'provider'];
...
router.post('/login', rateLimit, async (req, res, next) => {
    try {
        ...
        if (!email || !password)
            throw new ApiError(400, 'يرجى إدخال البريد وكلمة المرور');
        if (String(password).length > 72)
            throw new ApiError(400, 'كلمة المرور طويلة جداً');
        ...
        return ok(res, { token, csrf_token: csrfToken, user: enrichUser(user) });
    }
    catch (e) {
        next(e);
    }
...
router.post('/2fa/verify', rateLimit, async (req, res, next) => {
    try {
        ...
        if (!twofa_token || !code)
            throw new ApiError(400, 'رمز التحقق مطلوب');
        let payload;
        try {
        ...
        if (!user || !user.totp_enabled || !user.totp_secret)
            throw new ApiError(400, 'المصادقة الثنائية غير مفعّلة لهذا الحساب');
        if (!user.is_active)
...
router.post('/2fa/setup', authenticate, (req, res, next) => {
    try {
        ...
        // يُحفظ السر فوراً (حالة pending) ولا يُفعَّل حتى يثبت المستخدم امتلاكه للرمز
        run('UPDATE users SET totp_secret = ?, updated_at = datetime(\'now\') WHERE id = ?', [secret, user.id]);
        return ok(res, {
            secret,
            otpauth_uri: uri,
...
router.post('/2fa/enable', authenticate, (req, res, next) => {
    try {
        ...
        const user = get('SELECT * FROM users WHERE id = ?', [req.user.id]);
        if (!user.totp_secret)
            throw new ApiError(400, 'ابدأ خطوة الإعداد أولاً (setup)');
        if (!verifyCode(user.totp_secret, code))
            throw new ApiError(401, 'رمز التحقق غير صحيح');
...
router.post('/2fa/disable', authenticate, (req, res, next) => {
    try {
        ...
        const user = get('SELECT * FROM users WHERE id = ?', [req.user.id]);
        if (!user.totp_enabled)
            throw new ApiError(400, 'المصادقة الثنائية غير مفعّلة');
        if (!verifyCode(user.totp_secret, code || ''))
            throw new ApiError(401, 'رمز التحقق غير صحيح');
...
router.post('/2fa/reset', authenticate, async (req, res, next) => {
    try {
        if (req.user.role !== 'admin')
            throw new ApiError(403, 'متاح للمسؤول فقط');
        const user_id = Number(req.body && req.body.user_id);
        if (!Number.isInteger(user_id) || user_id <= 0)
            throw new ApiError(400, 'معرف الحساب غير صحيح');
        const target = get('SELECT id, role FROM users WHERE id = ?', [user_id]);
        if (!target)
            throw new ApiError(404, 'الحساب غير موجود');
...
router.post('/change-password', authenticate, csrfProtect, async (req, res, next) => {
    try {
        ...
        if (!current_password || !new_password)
            throw new ApiError(400, 'يرجى إدخال كلمة المرور الحالية والجديدة');
...
router.post('/register-customer', rateLimit, async (req, res, next) => {
    try {
        ...
        if (govId && !get('SELECT id FROM governorates WHERE id = ?', [govId]))
        ...
        if (refCode) {
            ...
            if (!referrer)
                throw new ApiError(400, 'كود الإحالة غير صالح');
            referredBy = referrer.id;
        }
        const passwordHash = await hashPassword(password);
        let userId, verifyToken;
        transaction(() => {
            userId = run('INSERT INTO users (role, name_ar, email, phone, password_hash, governorate_id, is_active, is_verified, referred_by) VALUES (?,?,?,?,?,?,1,0,?)', ['customer', name_ar, cleanEmail, phone, passwordHash, govId, referredBy]).lastId;
...
router.post('/verify-email', rateLimit, (req, res, next) => {
    try {
        ...
        if (!token)
            throw new ApiError(400, 'رمز التأكيد مطلوب');
        const row = get('SELECT * FROM user_verifications WHERE token = ?', [String(token)]);
        if (!row)
            throw new ApiError(400, 'رمز التأكيد غير صالح أو مستخدم مسبقاً');
        if (new Date(row.expires_at) < new Date())
            throw new ApiError(400, 'انتهت صلاحية رمز التأكيد');
...
router.post('/reset-password', authenticate, requireAgentLease(), csrfProtect, async (req, res, next) => {
    try {
        const { user_id, new_password } = req.body || {};
        if (!user_id || !new_password)
            throw new ApiError(400, 'يرجى تحديد الحساب وكلمة المرور الجديدة');
        assertLength(new_password, 72, 'كلمة المرور', 6);
        const target = get('SELECT * FROM users WHERE id = ?', [Number(user_id)]);
        if (!target)
            throw new ApiError(404, 'الحساب غير موجود');
        if (target.role === 'admin' && req.user.role !== 'admin')
            throw new ApiError(403, 'لا يمكنك تغيير كلمة مرور المسؤول');
...
module.exports.enrichUser = enrichUser;
