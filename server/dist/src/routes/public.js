...
const { ApiError, toId, round2, paginate, parseImages, parseIncludes } = require('../utils/helpers');
...
// GET /api/public/payment-info — حسابات استقبال شحن رصيد المزودين (تُعرض لمزود الخدمة قبل الدفع)
// يُسمح بأكثر من رقم/آيبان لكل قناة: يُكتب كل عنصر في سطر مستقل، وتُعاد كقائمة.
// مصادقة واشتراط دور المزود: هذه بيانات مالية خاصة لا تُكشف للزوار.
router.get('/payment-info', authenticate, requireRole('provider'), (req, res, next) => {
    try {
...
// أعمدة عامة صريحة فقط — لا تستخدم p.* ولا تعيد مستندات التوثيق
// (national_id_image/residency_doc_image) ولا الحقول الداخلية للمزوّد.
const PROVIDER_SELECT = `
  ...
  WHERE p.is_active = 1
`;
// GET /api/public/governorates — المحافظات النشطة (أعمدة عامة فقط: لا نسرب lease_fee)
router.get('/governorates', (req, res, next) => {
    try {
        const rows = all(`SELECT g.id, g.name_ar, g.name_en, g.code, g.sort_order,
...
// GET /api/public/coupons — الكوبونات النشطة غير المنتهية لصفحة "كوبونات وعروض".
// تُعاد نسبة/مبلغ الخصم النصي ليُعرض كأرقام كبيرة في بطاقات الكوبون.
router.get('/coupons', (req, res, next) => {
    try {
        const rows = all(`SELECT c.id, c.code, c.title, c.discount_type, c.discount_value, c.min_amount, c.provider_id,
       ...
       ORDER BY c.discount_value DESC, c.id DESC`).map((c) => ({
            id: c.id,
            code: c.code,
            title: c.title,
...
// GET /api/public/config — إعدادات عامة للتطبيق (عتبة الشحن المجاني ... إلخ)
...
// GET /api/public/services?governorate_code= — الخدمات النشطة مع عدد مزوديها في المحافظة
router.get('/services', (req, res, next) => {
    try {
        const { governorate_code } = req.query;
        let govId = null;
        if (governorate_code) {
        ...
        const rows = all(`SELECT s.id, s.slug, s.name_ar, s.name_en, s.description, s.icon, s.sort_order,
              COUNT(p.id) AS providers_count
       FROM services s
       LEFT JOIN providers p ON p.service_id = s.id AND p.is_active = 1 ${govId ? 'AND p.governorate_id = ?' : ''}
       WHERE s.is_active = 1
       GROUP BY s.id ORDER BY s.sort_order ASC`, govId ? [govId] : []).map((s) => ({ ...s, providers_count: Number(s.providers_count) || 0 }));
        return ok(res, rows);
    }
    catch (e) {
        next(e);
...
// GET /api/public/providers?governorate_code=&service_slug=&q=&min_rating=&verified=&sort=&limit=
router.get('/providers', (req, res, next) => {
    try {
        const { governorate_code, service_slug, q, verified, sort, min_rating } = req.query;
        const conditions = [];
        const params = [];
        if (governorate_code) {
            conditions.push('g.code = ?');
            params.push(String(governorate_code).toUpperCase());
        }
        if (service_slug) {
        ...
        const orderBy = sort === 'recent' ? 'p.id DESC' :
        ...
        // أعداد الكتالوج لكل مزود دفعة واحدة (GROUP BY بدل N+1 لكل مزود)
        const countsMap = {};
        if (rows.length) {
            const ids = rows.map((r) => r.id).join(',');
            const countByTable = (table) => all(`SELECT provider_id, COUNT(*) AS c FROM ${table} WHERE provider_id IN (${ids}) AND is_active = 1 GROUP BY provider_id`);
            for (const table of ['products', 'menu_items', 'hotel_rooms', 'flights', 'travel_packages']) {
                for (const row of countByTable(table))
                    countsMap[`${row.provider_id}:${table}`] = row.c;
            }
        }
...
// GET /api/public/providers/:id — تفاصيل مزود مع أعداد الفهرسة
router.get('/providers/:id', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const p = get(PROVIDER_SELECT + ' AND p.id = ?', [id]);
        if (!p)
            throw new ApiError(404, 'مزود الخدمة غير موجود');
        const slug = p.service_slug;
        const counts = {
            products: slug === 'stores' ? get('SELECT COUNT(*) AS c FROM products WHERE provider_id = ? AND is_active = 1', [id]).c : 0,
            menu_items: slug === 'restaurants' ? get('SELECT COUNT(*) AS c FROM menu_items WHERE provider_id = ? AND is_active = 1', [id]).c : 0,
            rooms: slug === 'hotels' ? get('SELECT COUNT(*) AS c FROM hotel_rooms WHERE provider_id = ? AND is_active = 1', [id]).c : 0,
            flights: slug === 'flights' ? get('SELECT COUNT(*) AS c FROM flights WHERE provider_id = ? AND is_active = 1', [id]).c : 0,
            packages: slug === 'travel_offices' ? get('SELECT COUNT(*) AS c FROM travel_packages WHERE provider_id = ? AND is_active = 1', [id]).c : 0,
        };
        const followers_count = get('SELECT COUNT(*) AS c FROM provider_follows WHERE provider_id = ?', [id]).c;
        return ok(res, { ...publicProvider(p), catalog_counts: counts, followers_count });
    }
    catch (e) {
        next(e);
    }
});
// GET /api/public/providers/:id/reviews — مراجعات وتقييمات الزبائن (مع ردود المزود)
router.get('/providers/:id/reviews', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const p = get('SELECT id FROM providers WHERE id = ?', [id]);
        if (!p)
            throw new ApiError(404, 'مزود الخدمة غير موجود');
        const limit = Math.min(Number(req.query.limit) || 20, 50);
        const rows = all(`SELECT r.id, r.rating, r.comment, r.created_at, r.reply, r.replied_at,
       ...
       LIMIT ?`, [id, limit]);
        const breakdown = all('SELECT rating, COUNT(*) AS count FROM provider_ratings WHERE provider_id = ? GROUP BY rating', [id]);
        return ok(res, { reviews: rows, breakdown });
    }
    catch (e) {
        next(e);
    }
});
// GET /api/public/providers/:id/categories — أقسام المنتجات النشطة (تاجر)
router.get('/providers/:id/categories', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const rows = all(`SELECT c.id, c.name_ar, c.name_en, c.icon, c.sort_order,
              (SELECT COUNT(*) FROM products p WHERE p.category_id = c.id AND p.is_active = 1) AS items_count
       FROM product_categories c WHERE c.provider_id = ? AND c.is_active = 1 ORDER BY c.sort_order ASC, c.id ASC`, [id]);
        return ok(res, rows);
    }
    catch (e) {
        next(e);
...
// GET /api/public/providers/:id/products?category_id=&offers=1&min_price=&max_price=&sort=
router.get('/providers/:id/products', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const { category_id, offers, min_price, max_price, sort } = req.query;
        ...
        if (offers !== undefined) {
            sql += ' AND (p.is_featured = 1 OR (p.old_price IS NOT NULL AND p.old_price > p.price))';
        }
        ...
        if (offers !== undefined)
            totalSql += ' AND (is_featured = 1 OR (old_price IS NOT NULL AND old_price > price))';
        if (!Number.isNaN(lo) && lo > 0)
            totalSql += ' AND price >= ?';
        if (!Number.isNaN(hi) && hi > 0)
            totalSql += ' AND price <= ?';
        ...
        const orderBy = sort === 'price_asc' ? 'p.price ASC, p.id DESC' :
            sort === 'price_desc' ? 'p.price DESC, p.id DESC' :
                sort === 'rating' ? 'COALESCE(irs.rating,0) DESC, p.id DESC' :
                    'p.is_featured DESC, p.id DESC';
        let rows = all(sql + ' ORDER BY ' + orderBy + ' LIMIT ? OFFSET ?', [...params, pg.limit, pg.offset]).map((r) => ({
            ...r,
        ...
        if (sort === 'sold')
            rows = rows.sort((a, b) => b.sold - a.sold || b.id - a.id);
...
// GET /api/public/providers/:id/menu — قائمة المطعم النشطة
router.get('/providers/:id/menu', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const items = all(`SELECT m.id, m.provider_id, m.category_id, m.name_ar, m.name_en, m.description,
       ...
       ORDER BY m.is_featured DESC, m.id DESC`, [id]).map((r) => ({ ...r, images: parseImages(r.images_json) }));
        return ok(res, items);
    }
    catch (e) {
        next(e);
...
// GET /api/public/providers/:id/rooms
router.get('/providers/:id/rooms', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const rows = all(`SELECT hr.id, hr.provider_id, hr.name_ar, hr.name_en, hr.description, hr.price_per_night, hr.room_type, hr.max_guests, hr.images_json, hr.is_active, hr.is_featured,
              COALESCE(irs.rating, 0) AS rating, COALESCE(irs.rating_count, 0) AS rating_count,
              pr.name_ar AS provider_name
       FROM hotel_rooms hr
       ...
       WHERE hr.provider_id = ? AND hr.is_active = 1 ORDER BY hr.price_per_night ASC`, [id]).map((r) => ({ ...r, images: parseImages(r.images_json) }));
        return ok(res, rows);
    }
    catch (e) {
        next(e);
...
// GET /api/public/providers/:id/flights
router.get('/providers/:id/flights', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const rows = all(`SELECT f.id, f.provider_id, f.flight_number, f.origin, f.origin_ar, f.destination, f.destination_ar,
       ...
       ORDER BY CASE WHEN f.departure_at IS NULL OR f.departure_at = '' THEN 1 ELSE 0 END, f.departure_at ASC`, [id]).map((r) => ({ ...r, images: parseImages(r.images_json) }));
        return ok(res, rows);
    }
    catch (e) {
        next(e);
...
// GET /api/public/providers/:id/packages
router.get('/providers/:id/packages', (req, res, next) => {
    try {
        const id = toId(req.params.id);
        const rows = all(`SELECT p.id, p.provider_id, p.name_ar, p.name_en, p.description, p.destination, p.duration_days, p.price, p.includes_json, p.images_json, p.is_active, p.is_featured,
              COALESCE(irs.rating, 0) AS rating, COALESCE(irs.rating_count, 0) AS rating_count,
              pr.name_ar AS provider_name
       FROM travel_packages p
       LEFT JOIN providers pr ON pr.id = p.provider_id
       LEFT JOIN item_rating_sums irs ON irs.item_type = 'packages' AND irs.item_id = p.id
       WHERE p.provider_id = ? AND p.is_active = 1 ORDER BY p.id DESC`, [id]).map((r) => ({ ...r, images: parseImages(r.images_json), includes: parseIncludes(r.includes_json) }));
        return ok(res, rows);
    }
    catch (e) {
        next(e);
...
// GET /api/public/items/:kind/:id/reviews — مراجعات وتقييمات البند (معدل + تفصيل النجوم + قائمة المراجعات)
router.get('/items/:kind/:id/reviews', (req, res, next) => {
    try {
        ...
        const limit = Math.min(Number(req.query.limit) || 20, 50);
        const rows = all(`SELECT r.id, r.rating, r.comment, r.created_at,
              u.name_ar AS customer_name, u.avatar AS customer_avatar
       FROM item_ratings r
       JOIN users u ON u.id = r.customer_id
       WHERE r.item_type = ? AND r.item_id = ?
       ORDER BY r.created_at DESC, r.id DESC
       LIMIT ?`, [kind, itemId, limit]);
        const sum = get('SELECT rating, rating_count FROM item_rating_sums WHERE item_type = ? AND item_id = ?', [kind, itemId]);
        const breakdown = all('SELECT rating, COUNT(*) AS count FROM item_ratings WHERE item_type = ? AND item_id = ? GROUP BY rating', [kind, itemId]);
        return ok(res, {
            item_type: kind,
            item_id: itemId,
...
// GET /api/public/deals — عروض اليوم عبر الخدمات الخمس:
// منتجات عليها خصم (صفقات سريعة) + عناصر مميزة من المطاعم/الفنادق/الطيران/مكاتب السفر.
// يعيد بنوداً موحّدة تُبنى روابطها في التطبيق حسب kind.
router.get('/deals', (req, res, next) => {
    try {
        const limit = Math.min(Number(req.query.limit) || 12, 50);
        // فلتر المحافظة: عروض مزودي المحافظة الحالية فقط (عند تمرير الرمز)
        const { governorate_code } = req.query;
        let govId = null;
        if (governorate_code) {
            const gov = get('SELECT id FROM governorates WHERE code = ? AND is_active = 1', [String(governorate_code).toUpperCase()]);
        ...
        const soldMap = buildSoldMap();
        const products = all(`SELECT p.id AS item_id, p.provider_id, p.name_ar, p.price, p.old_price, p.images_json,
       ...
       LIMIT ?`, [...govParam, limit]);
        ...
        for (const f of featured) {
            const rows = all(`SELECT t.id AS item_id, t.provider_id, t.${f.titleCol} AS title, t.${f.priceCol} AS price,
