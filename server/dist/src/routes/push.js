...
router.get('/vapid-key', (req, res) => {
    if (!publicVapidKey)
        throw new ApiError(503, 'الإشعارات الفورية غير مفعّلة حالياً');
...
router.post('/subscribe', (req, res, next) => {
    try {
        ...
        if (!keys || !keys.p256dh || !keys.auth)
            throw new ApiError(400, 'مفاتيح الإشعار غير مكتملة');
        ...
        run(`INSERT INTO push_subscriptions (user_id, endpoint, keys_json, user_agent)
       VALUES (?,?,?,?)
       ON CONFLICT(endpoint) DO UPDATE SET
         keys_json = excluded.keys_json,
         user_agent = excluded.user_agent,
         last_seen_at = datetime('now')`, [req.user.id, endpoint, JSON.stringify({ p256dh: keys.p256dh, auth: keys.auth }), (req.headers['user-agent'] || '').slice(0, 300)]);
...
router.post('/unsubscribe', (req, res, next) => {
    try {
        const endpoint = String((req.body && req.body.endpoint) || '');
        if (!endpoint)
            throw new ApiError(400, 'نقطة الاشتراك مطلوبة');
