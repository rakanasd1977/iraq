...
function validateCoupon(code, { providerId, amount, customerId }) {
    ...
    if (!raw)
        throw new ApiError(400, 'أدخل رمز الكوبون');
    const coupon = get('SELECT * FROM coupons WHERE code = ?', [raw]);
    if (!coupon || !coupon.is_active)
        throw new ApiError(400, 'رمز الكوبون غير صالح');
    ...
    if (subtotal <= 0)
        throw new ApiError(400, 'إجمالي الطلب غير صالح لتطبيق الكوبون');
    ...
    if (customerId) {
        const usedByCustomer = get('SELECT COUNT(*) AS c FROM coupon_usages WHERE coupon_id = ? AND customer_id = ?', [coupon.id, customerId]).c;
    ...
    if (discount <= 0)
        throw new ApiError(400, 'قيمة الخصم صفر، لا يمكن تطبيق الكوبون');
