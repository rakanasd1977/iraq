...
function validateImageBuffer(buf, mime) {
    ...
    if (!spec)
        throw new ApiError(400, 'صيغة الصورة غير مدعومة — يقبل PNG وJPG وWebP وGIF فقط');
...
function saveBase64ToUpload(dataUri, opts = {}) {
    const maxBytes = opts.maxBytes || MAX_BYTES;
    const m = /^data:([a-z]+\/[a-z0-9.+-]+);base64,(.+)$/i.exec(String(dataUri || '').trim());
    if (!m)
        throw new ApiError(400, 'صيغة غير صالحة — أرسل الصورة بصيغة data:image/...;base64');
    const mime = m[1].toLowerCase();
    if (!mime.startsWith('image/'))
        throw new ApiError(400, 'يُقبل ملفات الصور فقط');
    const buf = Buffer.from(m[2], 'base64');
    if (buf.length === 0)
        throw new ApiError(400, 'الملف فارغ');
    if (buf.length > maxBytes)
        throw new ApiError(400, `حجم الصورة يتجاوز الحد الأقصى (${Math.round(maxBytes / 1024 / 1024)}MB)`);
...
function deleteUploadValue(value) {
    for (const ref of extractUploadRefs(value))
        deleteUploadRef(ref);
}
...
function convertBase64Value(value) {
    ...
    if (s.startsWith('data:image/'))
        return saveBase64ToUpload(s).url;
    return value;
}
module.exports = {
    UPLOAD_DIR,
