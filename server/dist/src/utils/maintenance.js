...
// دمج WAL مع الملف الرئيسي ثم نسخ ذري عبر VACUUM INTO (لا يقطع خدمة النشاط)
function checkpointAndBackup() {
    try {
        ...
        console.log(`[maintenance] نسخة احتياطية: ${path.basename(target)}`);
        // حزمة كاملة (قاعدة البيانات + مجلد الصور) في ملف واحد للاستعادة الشاملة
        const bundle = createBackupBundle(target, stamp);
        if (bundle)
            console.log(`[maintenance] حزمة كاملة: ${path.basename(bundle)}`);
        pruneBackups();
        const toPush = bundle || target;
        if (OFFSITE_BACKUP_CMD)
            pushOffsiteBackup(toPush);
    }
    catch (e) {
...
function pruneIdempotencyKeys() {
    try {
        ...
        if (res?.changes > 0)
            console.log(`[maintenance] تقليم idempotency_keys: حُذف ${res.changes} مفتاحاً أقدم من ${days} يوم`);
    }
    catch (e) {
...
function endExpiredPromotions() {
    try {
        const expired = all(`SELECT pr.id, pr.item_title, pr.ends_at, u.id AS user_id, p.name_ar AS provider_name
       FROM promotions pr
       ...
       WHERE pr.status = 'active' AND pr.ends_at IS NOT NULL AND pr.ends_at <= datetime('now')`);
        ...
        if (expired.length)
            console.log(`[maintenance] إنهاء ترويجات منتهية: ${expired.length} + إشعار لكل مزود`);
        const expiring = all(`SELECT pr.id, pr.item_title, pr.ends_at, u.id AS user_id
       FROM promotions pr
       JOIN providers p ON p.id = pr.provider_id
       JOIN users u ON u.id = p.user_id
       WHERE pr.status = 'active' AND pr.ends_at > datetime('now')
        ...
        for (const pr of expiring) {
            ...
            const sent = get("SELECT id FROM notifications WHERE user_id = ? AND type = 'promotion_expiring' AND body = ?", [pr.user_id, body]);
            if (sent)
                continue;
            notifyUser(pr.user_id, {
        ...
        if (expiring.length)
            console.log(`[maintenance] تذكير انتهاء قريب: ${expiring.length} إعلاناً خلال 24 ساعة`);
    }
    catch (e) {
...
function rotateLogs() {
    for (const name of LOG_FILES) {
        ...
        try {
            ...
            if (e.code !== 'ENOENT')
                console.error(`[maintenance] rotate ${name} failed:`, e.message);
        }
    }
}
...
async function checkAdminNotifications() {
    try {
        ...
        const pendingWithdrawals = all(`SELECT aw.id, aw.amount, aw.created_at, a.name_ar AS agent_name, u.id AS user_id
       ...
       ORDER BY aw.created_at ASC`);
        for (const w of pendingWithdrawals) {
            const alreadyNotified = get("SELECT id FROM notifications WHERE user_id = ? AND type = 'withdrawal_pending' AND body LIKE ?", [0, `%السحب #${w.id}%`] // user_id=0 placeholder, will use role
            );
            // سنستخدم notifyRole للأدمنز
            const exists = get("SELECT id FROM notifications WHERE type = 'withdrawal_pending' AND body LIKE ?", [`%السحب #${w.id}%`]);
            if (!exists) {
        ...
        const pendingProviders = all(`SELECT p.id, p.name_ar, p.created_at, u.id AS user_id
       FROM providers p
       JOIN users u ON u.id = p.user_id
       WHERE (p.is_active = 0 OR p.is_verified = 0) AND p.created_at <= datetime('now', '-3 day')
       ORDER BY p.created_at ASC`);
        for (const p of pendingProviders) {
            const exists = get("SELECT id FROM notifications WHERE type = 'provider_pending' AND body LIKE ?", [`%المزود #${p.id}%`]);
            if (!exists) {
                await notifyRole('admin', {
        ...
        const expiringCoupons = all(`SELECT c.id, c.code, c.ends_at, p.name_ar AS provider_name, u.id AS user_id
       FROM coupons c
       ...
       ORDER BY c.ends_at ASC`);
        for (const c of expiringCoupons) {
            ...
            const exists = get("SELECT id FROM notifications WHERE type = 'coupon_expiring' AND body = ?", [body]);
            if (!exists) {
                await notifyRole('admin', {
        ...
        const expiredCoupons = all(`SELECT c.id, c.code, p.name_ar AS provider_name
       FROM coupons c
       JOIN providers p ON p.id = c.provider_id
       WHERE c.is_active = 1 AND c.ends_at IS NOT NULL AND c.ends_at <= datetime('now')`);
...
module.exports = { checkpointAndBackup, createBackupBundle, pushOffsiteBackup, pruneBackups, pruneIdempotencyKeys, endExpiredPromotions, rotateLogs, maintenanceSnapshot, checkAdminNotifications, BACKUP_KEEP, BACKUP_DIR, OFFSITE_BACKUP_CMD, UPLOAD_DIR };
