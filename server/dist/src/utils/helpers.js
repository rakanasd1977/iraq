...
function toId(value, name = 'المعرف') {
    ...
    if (!Number.isInteger(id) || id <= 0)
        throw new ApiError(400, `قيمة ${name} غير صحيحة`);
    return id;
}
function round2(n) {
...
function assertLength(value, max, name, min = 1) {
    ...
    if (s.length < min)
        throw new ApiError(400, `قيمة ${name} قصيرة جداً`);
    if (s.length > max)
        throw new ApiError(400, `قيمة ${name} تتجاوز الحد المسموح (${max} حرفاً)`);
    return s;
}
...
function parseDateRange(from, to) {
    ...
    if (from !== undefined && from !== null && from !== '') {
        ...
        if (!fromUtc)
            throw new ApiError(400, 'تاريخ البداية غير صالح (الصيغة YYYY-MM-DD)');
    }
    if (to !== undefined && to !== null && to !== '') {
        toUtc = localDayUtcBoundary(to, true);
        if (!toUtc)
            throw new ApiError(400, 'تاريخ النهاية غير صالح (الصيغة YYYY-MM-DD)');
    }
    if (fromUtc && toUtc && fromUtc > toUtc)
        throw new ApiError(400, 'تاريخ البداية يسبق تاريخ النهاية');
    return { fromUtc, toUtc };
}
module.exports = { ApiError, toId, round2, nextLeasePeriod, settingValue, paginate, csvEscape, assertLength, isUniqueViolation, localDayUtcBoundary, parseDateRange, parseImages, parseIncludes };
