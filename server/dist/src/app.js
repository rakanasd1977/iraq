...
function imageBytesMatch(buf, mime) {
    ...
    if (!spec || buf.length < spec.sig.length || !spec.sig.every((b, i) => buf[i] === b))
        return false;
...
app.use('/uploads', (req, res, next) => {
    ...
    // مستندات الهوية تُقدَّم عبر المسار المصادق فقط — لا تُكشف عامة بمعرفة الرابط
    const ref = '/uploads/' + path.basename(abs);
    const identity = get('SELECT 1 AS one FROM providers WHERE national_id_image = ? OR residency_doc_image = ? LIMIT 1', [ref, ref]);
    if (identity)
        return res.status(404).json({ message: 'الملف غير موجود' });
